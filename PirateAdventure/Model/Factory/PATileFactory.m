...
//    [[PATile alloc] init], [[PATile alloc] init], nil];
    ...
    [usedTiles addObject:[NSNumber numberWithBool:YES]];
    for (int i= 1; i < storyTiles.count; i++) {
        [usedTiles addObject:[NSNumber numberWithBool:NO]];
    }
    ...
    for (int i=0; i < storyTiles.count - 1; i++) {
        //retrieve a random integer between 0 and 12
        ...
        while([[usedTiles objectAtIndex:randomStoryIndex] boolValue] == YES) {
            NSLog(@"Random index %i already exists in the used tiles array. Generating a new one.", randomStoryIndex);
            if (randomStoryIndex + 1 >= storyTiles.count) {
                randomStoryIndex = 1;
            } else {
                randomStoryIndex += 1;
            }
            NSLog(@"The new random integer is %i", randomStoryIndex);
        }
